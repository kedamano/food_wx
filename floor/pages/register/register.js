...
Page({
  ...
  onVerifyCodeInput: function(e) {
    // 限制只能输入数字
    var verifyCode = e.detail.value.replace(/\D/g, '');
    this.setData({
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        if (res.statusCode === 200 && res.data && res.data.code === 200) {
          ...
          var errorMessage = res.data && res.data.message ? res.data.message : '发送验证码失败';
  ...
  onWeChatRegister: function() {
    ...
    wx.login({
      success: function(res) {
        ...
        if (res.code) {
          // 发送到后端进行微信注册
          self.weChatRegisterToBackend(res.code);
        } else {
          wx.showToast({
            title: '微信登录失败',
            icon: 'none'
  ...
  // 微信注册后端验证
  weChatRegisterToBackend: function(code) {
    var self = this;
    wx.request({
      ...
      success: function(res) {
        if (res.statusCode === 200 && res.data.code === 200) {
          // 注册成功
          self.handleRegisterSuccess(res.data.data);
        } else {
          self.showError(res.data.message || '微信注册失败');
        }
      },
      fail: function(err) {
  ...
  validateForm: function() {
    ...
    var usernameRegex = /^[a-zA-Z0-9_\u4e00-\u9fa5]+$/;
      ...
      success: function(res) {
        ...
        if (res.statusCode === 200) {
          if (res.data.code === 200) {
            ...
            // 注册失败
            self.showError(res.data.message || '注册失败');
          }
        } else {
          self.showError('网络请求失败');
        }
      },
      fail: function(err) {
